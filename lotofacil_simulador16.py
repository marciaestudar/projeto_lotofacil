# -*- coding: utf-8 -*-
"""lotofacil_simulador16.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gTTcZv_KH5yjxb7zudApkK3SQ_DsJ-sn
"""
...
st.set_page_config(
    ...
    page_icon="🍀", # Emojii alterado para trevo
    # layout="wide" # REMOVIDO/COMENTADO para melhor visualização em celulares
)
...
def carregar_dados_oficiais():
    ...
    if os.path.exists(ARQUIVO_HISTORICO_OFICIAL_CSV):
        try:
            ...
            # --- REMOVIDO: Linha de diagnóstico de data bruta, não é mais necessária aqui ---
            # print(f"Primeiras 5 datas brutas da coluna 'Data Sorteio':\n{df['Data Sorteio'].head().to_string(index=False)}")
            # ----------------------------------------------------------------------
...
with tab1:
    ...
    if not df_meus_simulados.empty:
        with st.form("form_conferencia_jogo_historico"):
            ...
            # Dica para concisão: o Streamlit já ajusta o tamanho da caixa de texto, mas manter o texto de exemplo curto ajuda
            numeros_oficiais_str = st.text_input(
                "Números Oficiais (ex: 1, 3, 5, ..., 25)", # Exemplo mais conciso
...
with tab2:
    ...
    if not df_oficial.empty:
        # Usando st.expander para o histórico completo
        with st.expander("Ver Histórico Completo de Sorteios Oficiais"):
            ...
            if 'Data Sorteio' in df_display.columns:
                df_display['Data Sorteio'] = df_display['Data Sorteio'].dt.strftime('%d/%m/%Y').fillna('Data Inválida')
        ...
        # Usando st.expander para a frequência dos números
        with st.expander("Ver Frequência dos Números Oficiais"):
...
with tab3:
    ...
    with st.form("form_novo_concurso"):
        col1, col2 = st.columns(2) # Colunas são responsivas e se empilham em mobile
        with col1:
        ...
        bolas = []
        for row in range(3):
            cols_bolas = st.columns(5) # As 5 colunas também se empilharão em telas muito estreitas
            for i in range(5):
        ...
        if submitted:
            ...
            if len(set(bolas)) != 15:
                ...
                novo_sorteio_dict = {
                    ...
                    'Data Sorteio': data_sorteio.strftime("%Y-%m-%d") # Formato AAAA-MM-DD para salvar no CSV
                }
